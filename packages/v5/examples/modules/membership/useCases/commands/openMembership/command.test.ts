...
describe("createOpenMembershipCommand", () => {
  ...
  it("returns a command with a unique id on each call", () => {
    const first = createOpenMembershipCommand(aggregateId, VALID_PAYLOAD, VALID_METADATA);
    const second = createOpenMembershipCommand(aggregateId, VALID_PAYLOAD, VALID_METADATA);
