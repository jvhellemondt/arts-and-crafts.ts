...
describe("correlationIdFromHeaders", () => {
  it("returns the x-correlation-id header value when present", () => {
    expect(correlationIdFromHeaders({ "x-correlation-id": "abc-123" })).toBe("abc-123");
  });

  it("generates a UUIDv7 when the header is absent", () => {
    expect(correlationIdFromHeaders({})).toMatch(UUID_V7_PATTERN);
  });
  ...
  it("uses a custom idFactory when provided", () => {
    expect(correlationIdFromHeaders({}, { idFactory: () => "fixed-id" })).toBe("fixed-id");
  });
});
